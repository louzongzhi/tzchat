...
class tzchatLinear(Linear):
    def __init__(self,
                 ...
                 keep_prob=1.0):
        super(tzchatLinear, self).__init__(
            in_channels,
            out_channels,
            bias_init=bias_init,
            has_bias=has_bias,
...
class tzchatFeedForward(Cell):
    @_LogActionOnce(m_logger=logger, key='tzchatFeedForward',
                    no_warning=_get_parallel_mode() in (ParallelMode.STAND_ALONE,))
    ...
    def __init__(self, dim,
        ...
        if self.ffn_concat:
            self.w_gate_hidden = tzchatLinear(in_channels=dim,
            ...
            self.w1 = tzchatLinear(in_channels=dim,
            ...
            self.w3 = tzchatLinear(in_channels=dim,
                                     out_channels=hidden_dim,
                                     has_bias=False,
                                     sigma=sigma,
                                     mean=mean,
        ...
        self.w2 = tzchatLinear(in_channels=hidden_dim,
                                 out_channels=dim,
                                 has_bias=True,
                                 sigma=sigma,
                                 mean=mean,
