...
from tzchat_transformer import tzchatDecodeLayer
from tzchat_interleave import tzchatDecodeLayerInterleave
from tzchat_layer import tzchatEmbedding
from tzchat_config import tzchatConfig


class tzchatPreTrainedModel(PreTrainedModel):
    config_class = tzchatConfig
    base_model_prefix = "tzchat"


class tzchatModel(tzchatPreTrainedModel):
    def __init__(self,
                 config: tzchatConfig = None):
        ...
        self.tok_embeddings = tzchatEmbedding(vocab_table_size=config.vocab_size,
                                                sigma=config.sigma,
                                                mean=config.mean,
        ...
        for layer_id in range(config.num_layers):
            if self.fine_grain_interleave:
                layer = tzchatDecodeLayerInterleave(config.seq_length,
                                                      layer_id,
                ...
                layer = tzchatDecodeLayer(layer_id,
                                            dim=config.hidden_size,
                                            n_heads=config.num_heads,
                                            n_kv_heads=config.n_kv_heads,
                                            sigma=config.sigma,
...
class tzchatForCausalLM(tzchatPreTrainedModel):
    @lazy_inline
    def __init__(self, config: tzchatConfig = None):
        super(tzchatForCausalLM, self).__init__(config, auto_prefix=True)
        _check_config(config.parallel_config)
        self.config = config
        ...
        self.model = tzchatModel(config=config)
        ...
        if config.quant == "w8a16":
            logger.info("Using RoundToNearest to quant tzchatForCausalLM.")
    ...
    def set_dynamic_inputs(self, **kwargs):
        ...
        logger.info("Set dynamic input for tzchat.")
